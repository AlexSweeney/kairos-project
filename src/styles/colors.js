export const whiteBackground = '#FFFFFF';
export const greyBackground = '#F4F4F4';
export const darkBlueBackground = '#181949';
export const lightBlueBackground = '#F3F9FF';
export const lightPinkBackground = '#FFD1E8';
export const pinkBackground = '#E6367E'; 

export const lightGreyBorder = '#DEDEDE';
export const pinkBorder = '#E6367E';

export const darkBlueText = '#181949';
export const lightBlueText = '#CFE5FF';
export const whiteText = '#FFFFFF';
export const greyText ='#666666';

export const lightGreyBoxShadow = 'rgba(0, 0, 0, 0.07)';