import React from 'react';
import {
	priceOptionsViewStyle,
	priceOptionsBoxStyle,
	priceOptionsBorderBoxStyle,
	priceOptionsHighlightBoxStyle,
	priceOptionBoxHeadingStyle,
	priceOptionBoxSubHeadingStyle,
	priceNumberStyle,
	priceZeroStyle,
	priceDollarStyle,
	priceTextStyle,
} from './../styles/PriceOptionsView.js';
import './../styles/PriceOptionsView.css';

export default function PriceOptionsView() {
	return (
		<section className="price-options-view blue-background" style={priceOptionsViewStyle}>
      <div className="price-option-box" style={priceOptionsBoxStyle}>
        <div className="price-option-box-row-1">
          <h3 style={priceOptionBoxHeadingStyle}>Free Test</h3>
          <h5 style={priceOptionBoxSubHeadingStyle}>Organize across all apps by hand</h5>
        </div>
        <div className="price-option-box-row-2">  
          <div className="price-left-col">
            <h3 style={priceZeroStyle}>0</h3>
          </div>
          <div className="price-right-col">
            <h3 style={priceDollarStyle}>$</h3>
            <h5 style={priceTextStyle}>Per Month</h5>
          </div>
        </div> 
      </div>
      <div className="price-option-box" style={priceOptionsBoxStyle}>
        <div className="price-option-box-row-1">
          <h3 style={priceOptionBoxHeadingStyle}>Low Price</h3>
          <h5 style={priceOptionBoxSubHeadingStyle}>Monthly Fixed Amount</h5>
        </div>
        <div className="price-option-box-row-2">  
          <div className="price-left-col">
            <h3 style={priceNumberStyle}>200.000</h3>
          </div>
          <div className="price-right-col">
            <h3 style={priceDollarStyle}>$</h3>
            <h5 style={priceTextStyle}>Per Month</h5>
          </div>
        </div> 
      </div>
      <div className="price-option-box price-option-box-border" style={priceOptionsBorderBoxStyle}>
        <div className="price-option-box-row-1">
          <h3 style={priceOptionBoxHeadingStyle}>Easy Using Methods</h3>
          <h5 style={priceOptionBoxSubHeadingStyle}>Various Manuals</h5>
        </div>
        <div className="price-option-box-row-2">  
          <div className="price-left-col">
            <h3 style={priceNumberStyle}>200.000</h3>
          </div>
          <div className="price-right-col">
            <h3 style={priceDollarStyle}>$</h3>
            <h5 style={priceTextStyle}>Per Month</h5>
          </div>
        </div> 
      </div>
      <div className="price-option-box price-option-box-highlight" style={priceOptionsHighlightBoxStyle}>
        <div className="price-option-box-row-1">
          <h3 style={priceOptionBoxHeadingStyle}>Verified IT Service</h3>
          <h5 style={priceOptionBoxSubHeadingStyle}>On sale in 4 countries</h5>
        </div>
        <div className="price-option-box-row-2">  
          <div className="price-left-col">
            <h3 style={priceNumberStyle}>300.000</h3>
          </div>
          <div className="price-right-col">
            <h3 style={priceDollarStyle}>VND</h3>
            <h5 style={priceTextStyle}>Per Month</h5>
          </div>
        </div> 
      </div>
    </section>
	)
} 
